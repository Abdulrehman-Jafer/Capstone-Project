...
export const Image = ({ url, id, isFavorite }: apiDataType) => {
  ...
  const HeartIcon = isFavorite ? (
    <AiFillHeart
      className={`absolute left-0 text-3xl rounded-[50%] text-red-400 cursor-pointer ${isFavorite ? "" : "hidden"
        }`}
      onClick={() => FavrtHandler(id)}
    />
  ) : (
    <AiOutlineHeart
      className={`absolute left-0 text-3xl rounded-[50%] text-red-400 cursor-pointer hover:text-red-500  ${hovered ? "" : "hidden"
        }`}
      onClick={() => FavrtHandler(id)}
    />
  );
  ...
  const cartOrPlusIcon = !alreadyCarted ? (
    <AiOutlinePlus
      className={`absolute right-0 text-3xl text-white bg-slate-400 hover:bg-slate-500 rounded-[20%] cursor-pointer ${hovered ? "" : "hidden"
        }`}
      onClick={() => {
        addToCart(id);
      }}
